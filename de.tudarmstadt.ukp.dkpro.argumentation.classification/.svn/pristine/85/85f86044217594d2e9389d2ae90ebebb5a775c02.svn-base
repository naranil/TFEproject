...
/**
 *
 * Use {@link #ProductionRulesCollector}
 * @author Christian Stab
 *
 */
public class ProductionRules extends FeatureExtractorResource_ImplBase implements MetaDependent, DocumentFeatureExtractor{
    ...
    public List<Feature> extract(JCas jcas) throws TextClassificationException {
        ...
        System.out.println("TITLE OF THE DOC "+ DocumentMetaData.get(jcas).getDocumentTitle());
        for (String docRule : rules.getKeys()){
...
//                featList.add(new Feature(FN_PRODUCTION_RULE + rule, rules.getCount(rule)));
                // Try binary
                featList.add(new Feature(FN_PRODUCTION_RULE + rule, 1));
